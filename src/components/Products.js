...
function Products({ activeCategoryId, searchQuery = '' }) {
    ...
    if (sessionStorage.getItem('scrollToProducts') === 'true') {
      ...
      setTimeout(() => {
        if (sectionRef.current) {
          const top = sectionRef.current.getBoundingClientRect().top + window.scrollY - 20
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(() => {
      ...
      if (activeCategoryId && activeCategoryId !== 'all') params.append('category', activeCategoryId)
  ...
  router.push(`/product-page/${p._id}`)
                ...
                >
                  ...
                  <div className="prod-card-body">
                    ...
                    <div className="prod-card-footer">
                      ...
                      <button className="prod-card-btn">{viewLabels[activeLang]}</button>
                    </div>
                  </div>
                </div>
              ))}
          ...
          {totalPages > 1 && (
            <div className="products-pagination">
              ...
              <span className="pag-info">{pageLabels[activeLang]} {page} / {totalPages}</span>
